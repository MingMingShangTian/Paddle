 ...
 public:
  ...
  template <typename T1,
            typename std::enable_if<std::is_floating_point<T1>::value ||
                                    ...
                                    void>::type>
  HOSTDEVICE inline complex& operator=(T1 val) {
  ...
  template <typename T1>
  HOSTDEVICE explicit complex(
      const std::enable_if<std::is_same<T1, double>::value, complex<float>>&
          val)
      : real(static_cast<double>(val.real)),
        imag(static_cast<double>(val.imag)) {}
...
#else
  return complex<T>(std::tanh(std::complex<float>(a)));
#endif
}
