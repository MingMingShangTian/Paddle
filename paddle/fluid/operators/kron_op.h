...
using complex<float> = paddle::platform::complex<float>;
using complex<double> = paddle::platform::complex<double>;
...
template <>
struct KronGradElemFunctor<complex<float>> {
  KronGradElemFunctor(const complex<float>* dout, const complex<float>* A,
                      const complex<float>* B, complex<float>* dout_a,
                      complex<float>* dout_b, const int64_t* stride_dout,
                      const int64_t* stride_a, const int64_t* stride_b,
                      const int64_t* shape_b, const int64_t numel_a,
                      const int64_t numel_b, const int ndims)
      : dout_(dout),
  ...
  HOSTDEVICE void operator()(int64_t idx) {
    ...
    if (dout_a_) {
      size_t index_out_a = index_a * numel_b_ + index_b;
      dout_a_[index_out_a] =
          dout_[idx] * complex<float>(B_[index_b].real, -B_[index_b].imag);
    }
    if (dout_b_) {
      size_t index_out_b = index_b * numel_a_ + index_a;
      dout_b_[index_out_b] =
          dout_[idx] * complex<float>(A_[index_a].real, -A_[index_a].imag);
    }
  }

 private:
  const complex<float>* dout_;
  const complex<float>* A_;
  const complex<float>* B_;
  complex<float>* dout_a_;
  complex<float>* dout_b_;
  const int64_t* stride_dout_;
  const int64_t* stride_a_;
  const int64_t* stride_b_;
  const int64_t* shape_b_;
...
template <>
struct KronGradElemFunctor<complex<double>> {
  KronGradElemFunctor(const complex<double>* dout, const complex<double>* A,
                      const complex<double>* B, complex<double>* dout_a,
                      complex<double>* dout_b, const int64_t* stride_dout,
                      const int64_t* stride_a, const int64_t* stride_b,
                      const int64_t* shape_b, const int64_t numel_a,
                      const int64_t numel_b, const int ndims)
      : dout_(dout),
  ...
  HOSTDEVICE void operator()(int64_t idx) {
    ...
    if (dout_a_) {
      size_t index_out_a = index_a * numel_b_ + index_b;
      dout_a_[index_out_a] =
          dout_[idx] * complex<double>(B_[index_b].real, -B_[index_b].imag);
    }
    if (dout_b_) {
      size_t index_out_b = index_b * numel_a_ + index_a;
      dout_b_[index_out_b] =
          dout_[idx] * complex<double>(A_[index_a].real, -A_[index_a].imag);
    }
  }

 private:
  const complex<double>* dout_;
  const complex<double>* A_;
  const complex<double>* B_;
  complex<double>* dout_a_;
  complex<double>* dout_b_;
  const int64_t* stride_dout_;
  const int64_t* stride_a_;
  const int64_t* stride_b_;
  const int64_t* shape_b_;
